	...
	main.getAllEvents(function(dt){
		...
		if(dt.status==200){
			...
			if(dt.content.length==1){
				...
				main.hrefTo('https://h5.48.cn/2018apppage/mvpvote/list.html');
				//main.hrefTo('http://h5.snh48.com/2018apppage/mvpvote/list.html');
				// main.hrefTo('list.html');
			}else{
				$.each(dt.content,function(index,dl){
					html.push('<li infoId='+dl.infoId+' code='+dl.code+'>');
					html.push('<p class="ptit">'+dl.teamName+' '+dl.liveName+'</p>');
					html.push('<p class="ptime">'+getDate(dl.liveTime)+'</p>');	
					html.push('</li>');
				})
...
function addListeners(){
	...
	$('.novotebtn').click(function(){
		main.hrefTo("https://h5.48.cn/2018apppage/xwcardbuy/index_18.html");
	});
