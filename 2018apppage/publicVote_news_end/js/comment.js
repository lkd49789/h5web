...
(function(data) {
    ...
    //优质点评
    data.goodList = function(){
        $.getJSON("commentlist.json", function(data) {
            var html = [];
            $.each(data.content, function(index, dl) {
                var index = index+1;
                html.push('<li>');
                html.push('<img src="img/listsm_bg.png" class="listsm_bg">');
                html.push('<span class="avata" style="background-image:url('+dl.img_url+')"></span>');
                html.push('<div class="content">');
                html.push('<p class="name">'+dl.name+'</p>');
                html.push('<p class="texts"><em style="color:#fae4c1">'+dl.title+'</em>'+dl.texts+'</p>');
                html.push('</div>');
                html.push('<div class="chikenbtn"><img src="img/chiken.png"><em class="zannum">+1000</em></div>');
                html.push('</li>');
            })
