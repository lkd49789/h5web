if(localStorage.getItem("phone_0828")){
	console.log("done")
	$('.readright').show();
	$('.succ').hide();
	$('.readright .tp1').html(localStorage.getItem("phone_0828")+" 手机号码已经预约");
}else{
	$('.remindbox,.succ').show();
}
...
$('.timestarbox').on("click",function(){
	if(checkFromApp()){
		liveUrl("5b7e694d0cf2a9d5d2e400d8");
	}else{
	...
	if(flag==true){
		...
		main.subscribeInfo($(".phonenum").val(),$(".code").val(),function(dt){
			...
			if(dt.status == 200){
				...
				localStorage.setItem("phone_0828",$(".phonenum").val());
			}else{
				showTip(dt.message)
			}
		})
...
function exChangeEnd(){
	var EndTime= new Date('2018/09/01 17:30:00');
...
function endChangeTime(){
	var EndTime= new Date('2018/09/01 22:30:00');
    var NowTime = new Date();
    var t =EndTime.getTime() - NowTime.getTime();
    var d=0;
    var h=0;
