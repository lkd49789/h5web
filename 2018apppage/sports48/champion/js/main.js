(function(data) {
    ...
    data.getStatus = function(succ){
        $.ajax({
            url:  CONFIG.getLink()+"api/fans/v1/guess/index",
            type: "POST", 
            async:true,
            contentType: "application/json; charset=utf-8",
...
//               alert("eee");
    ...
    //保存用户竞猜信息
    data.saveGuess = function(_type,_list,succ){
        $.ajax({
            url:  CONFIG.getLink()+"api/fans/v1/guess/saveGuess",
            //url:  "http://192.168.0.18:8080/games/api/fans/v1/guess/saveGuess",
            type: "POST", 
            async:true,
            contentType: "application/json; charset=utf-8",
            beforeSend: function (request) {
