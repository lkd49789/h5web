...
'                               <ul class="mask_title">'+
'                                   <p class="all">常用</p>'+
'                                   <li code="86"><p>中国大陆(86)</p></li>'+
'                                   <li code="852"><p>中国香港(852)</p></li>'+
'                                   <li code="886"><p>中国台湾(886)</p></li>'+
'                                   <li code="1"><p>美国(1)</p></li>'+
'                                   <li code="81"><p>日本(81)</p></li>'+
'                               </ul>'+
'                               <ol class="mask_city">'+
'                                    {{list}}'+
'                               </ol>'+
...
var li='<li code={{code}}><span class="{{opacity}}">{{letter}}</span><p>{{country}}({{code1}})</p></li>';
var callback;
	...
	data.init = function(){
		...
		for(var i=0; i<countryArr.length;i++){
			...
			_li = _li.replace('{{code1}}',counstr[2]);

			
			_li = _li.replace('{{country}}',counstr[1]);
	...
	data.addListeners = function(){
		...
		$(".mask li").click(function(){
			...
			COUNTRY.close($(this).attr("code"),$(this).find("p").html());
	...
	//根据code获取国家名字
	data.getCountry = function(code){
		var countryName = ""
		for(var i=0; i<countryArr.length;i++){
			var counstr = countryArr[i].split("-")
			if(code == counstr[2]){
				//countryName = counstr[1]+"("+counstr[2]+")"+'<i class="mui-icon mui-icon-arrowdown"></i>'
				countryName = counstr[1]+"("+counstr[2]+")"+'<i class="mui-icon mui-icon-arrowdown"></i>'
			}
		}
		return countryName;
