...
(function(data) {
  ...
  //一期生出道4周年纪念
  data.topicNew_4th = function(title, succ, error) {
    $.ajax({
      async: true, 
      crossDomain: true,
      url: CONFIG.getPath()+"h5/topic/saveSpecial",
      type: "POST", 
      dataType: 'json', 
      data: {auth:encryption,title:title,userAvatar:user_pic,userNickname:user_name},
      timeout: 5000, 
      // contentType: "application/json;utf-8", 
      success:function (sus){
        succ(sus.message);
